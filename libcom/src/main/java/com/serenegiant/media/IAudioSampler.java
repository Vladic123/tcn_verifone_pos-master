...
public abstract class IAudioSampler {
	...
	public static AudioRecord createAudioRecord(
			final int source, final int sampling_rate, final int channels, final int format,
			final int buffer_size) {

		@AudioSource final int[] AUDIO_SOURCES = new int[]{
		...
		switch (source) {
			case 1:
				AUDIO_SOURCES[0] = MediaRecorder.AudioSource.MIC;
				break;        // 自動
			case 2:
				AUDIO_SOURCES[0] = MediaRecorder.AudioSource.CAMCORDER;
				break;    // 内蔵マイク
			case 3:
				AUDIO_SOURCES[0] = MediaRecorder.AudioSource.VOICE_COMMUNICATION;
				break;
			default:
				AUDIO_SOURCES[0] = MediaRecorder.AudioSource.MIC;
				break;        // 自動(UACのopenに失敗した時など)
		}
...
//		if (DEBUG) Log.v(TAG, "addMediaData:" + mAudioQueue.size());
	...
	public interface SoundSamplerCallback {
		 ...
		 */
		void onData(ByteBuffer buffer, int size, long presentationTimeUs);
