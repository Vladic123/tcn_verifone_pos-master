...
//		if (DEBUG) Log.v(TAG, "ColorPickerView:");
	 ...
	 * @param force 強制更新するかどうか
	 */
	protected void setColor(final int alpha, final float hue, float sat, final float val,
							final boolean force) {
...
//			if (DEBUG) Log.v(TAG, String.format("onLayout:(%d,%d)(%d,%d)", left, top, right, bottom));
	 ...
	 * @param height
	 */
	private final void drawTrackerHorizontal(final Canvas canvas, final float x, final float y,
											 final float height) {
	 ...
	 * @param width
	 */
	private final void drawTrackerVertical(final Canvas canvas, final float x, final float y,
										   final float width) {
