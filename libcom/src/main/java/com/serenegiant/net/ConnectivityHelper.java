	...
	public static boolean isWifiNetworkReachable(@NonNull final Context context) {
		...
		if (BuildCheck.isLollipop()) {
			if (BuildCheck.isMarshmallow()) {
				...
				@Nullable final NetworkCapabilities capabilities = manager.getNetworkCapabilities(network);    // API>=21
				@Nullable final NetworkInfo info = manager.getNetworkInfo(network);    // API>=21
...
//================================================================================
	...
	public interface ConnectivityCallback {
		 ...
		 */
		void onNetworkChanged(final int activeNetworkType);
